...
var EnvBotao = async (id, MR, yoshino, text1, desc1, X) => {
if(X.split("|")[1] != "0") {
yoshino.sendMessage(id, {image: {url: X.split("|")[1]}, caption: text1, mentions: [MR]})} else {yoshino.sendMessage(id, {text: text1, mentions: [MR]})}}
} else {
var EnvBotao = async(id, MR, yoshino, text1, desc1, X, TXT = [], vr) => {
...
yoshino.sendMessage(id, buttonMessage, {quoted: vr}).catch(e => {
...
// ENVIAR FIGU EM URL
const enviarfiguUrl = async (yoshino, from, link, mr) => {
ranp = getRandom('.gif');
rano = getRandom('.webp');
ini_buffer = `${link}`;
...
yoshino.sendMessage(from, {sticker: buff}, {quoted: mr}).catch(() => {
...
const RSM_FUNC = async(yoshino, nmrdn_dono2, hora120, upsert) => {
switch(hora120) {
...
yoshino.sendMessage(nmrdn_dono2, {text: txt});
rg_alg_ofc.cobrou = true;
rg_alg_ofc.data = parseInt(rg_alg_ofc.data.split("/")[0])+"/"+horadd;
fs.writeFileSync("./settings/media/rg_aluguel.json", JSON.stringify(rg_aluguel))
}}}}

const comand = (yoshino, info, prefix, isGroup, Res_SoGrupo, sender, pushname, command, reply, args, from, mentions, Res_SoAdm, Res_BotADM, isGroupAdmins, isBotGroupAdmins, upsert) => {
...
module.exports = {  P, fs, util, Boom, axios, linkfy, request, ms, ffmpeg, fetch, qrterminal, exec, spawn, yts, execSync, limitefll, moment, time, hora, date, infoSystem, RSM_FUNC, comand, addVote, delVote, getBuffer, convertSticker, fetchJson, fetchText, getBase64, createExif, addLimit, getLimit, upload, nit, addBanned, unBanned, BannedExpired, cekBannedUser, validmove, setGame, addComandosId, deleteComandos, getComandoBlock, getComandos, addComandos, palavrasANA, quizanimais, getpc, supre, wait, getExtension, generateMessageID, getGroupAdmins, getMembros, getRandom, banner, banner2, banner3, temporizador, chyt, webp_mp4, simih, patentes, antispam, anotar, rg_aluguel, voting, sotoy, addVote, delVote, countMessage, comandos, daily, muted, nescessario, premium, ban, black_, joguinhodavelhajs, joguinhodavelhajs2, setting, logoslink, linguagem, inform, mess, destrava, destrava2, tabela, conselhob, fatos, palavrasc, recognize, colors, cheerio, NodeCache, kyun, sendVideoAsSticker, sendImageAsSticker, sendVideoAsSticker2,sendImageAsSticker2, enviarfiguUrl, getFileBuffer, DLT_FL, sleep, ANT_LTR_MD_EMJ, convertBytes, EnvBotao, Youtube, x2, arcloud, EmojiAPI, emoji, infoClima, os, garticArchives, enigmaArchive, translate, insert, response, randomCantadas, isFiltered, addFilter }